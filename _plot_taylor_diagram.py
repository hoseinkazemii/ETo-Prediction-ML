...
def srl(obsSTD, s, r, l, fname):
  ...
  for j,i in enumerate(srlc):
  	dia.add_sample(i[0], i[1], label=i[2], marker='o',mec = colors[j % len(colors)], mfc = 'none', mew=1.6)
  spl = [p.get_label() for p in dia.samplePoints]
  legend = fig.legend(dia.samplePoints, spl, numpoints=1, prop=dict(size='small'), loc=[0.7,0.3])
...
plt.savefig("TaylorDiagram.tif", dpi=300, bbox_inches="tight")